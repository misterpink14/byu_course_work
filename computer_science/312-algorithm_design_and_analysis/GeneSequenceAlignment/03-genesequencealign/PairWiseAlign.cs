...
namespace GeneticsLab
{
    class PairWiseAlign
    {
        ...
        public int Align(ref GeneSequence sequenceA, ref GeneSequence sequenceB, ref ResultTable resultTableSoFar, int rowInTable, int columnInTable)
        {
            int result = 0;
        ...
        private int extraction(string aSequence, string bSequence)
        {
            ...
            {
                row2[0] = new Node((i + 1) * 5, prev); // set default value
                for (int j = 0; j < bLength; j++) // foreach col
                {
                    int j1 = j + 1;
                    int cost = min(
                        5 + row1[j1].value, // prev row, curr col
                    ...
                    row2[j1] = new Node(cost, p);
                }
                Node[] temp = row1;
                row1 = row2;
                row2 = temp; // new Node[bLength + 1];
            }
        ...
        private void setRowDefaults(ref Node[] row1, int bLength)
        {
            ...
            {
                row1[i] = new Node((i) * 5, prev);
                prev = row1[i];
            }
        }


        public string scoring(string aSequence, string bSequence)
        {
            int aLength = aSequence.Length;
            int bLength = bSequence.Length;
            ...
            return this.getValue(table[aLength, bLength]);
        }
