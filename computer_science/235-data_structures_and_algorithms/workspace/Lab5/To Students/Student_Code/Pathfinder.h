...
class Pathfinder : public PathfinderInterface
{
	private:
		...
		bool recursion(int x, int y, int z)
		{
			...
			if(x < 4 && solve[x+1][y][z] == 1)
			{
				path.push_back(tostr((x+1),y,z));
				solve[x][y][z] = 2;
			...
			if(y < 4 && solve[x][y+1][z] == 1 )
			{
				path.push_back(tostr(x,(y+1),z));
				solve[x][y][z] = 2;
				if(recursion(x,++y,z) == true)
				{
					return true;
			...
			if(z < 4 && solve[x][y][z+1] == 1 )
			{
				path.push_back(tostr(x,y,(z+1)));
				solve[x][y][z] = 2;
				if(recursion(x,y,++z) == true)
				{
					return true;
