...
void UserPlay(LinkedList& TeamA, LinkedList& TeamB)
{
	...
	while(TeamA.size() != 1 && TeamB.size() != 1)
	{
		...
		std::cout << "Red Rover, Red Rover, send " << A->item.name << " right over!\n";
		if(B->next == NULL)
		{
		...
		std::cout << "Red Rover, Red Rover, send " << B->item.name << " right over!\n";
