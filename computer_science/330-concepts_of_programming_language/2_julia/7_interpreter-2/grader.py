...
println(testerr(lexParse, "(with (x 1) x)")) #Error
println(testerr(lexParse, "(with ((x 1) (x 1)) 1)")) #Error
println(testerr(lexParse, "(lambda (x x) 1)")) #Error

println(testerr(parseInter, "(- 1 2)")) #numVal -1
println(testerr(parseInter, "(with ((x 1)) x)")) # numVal 1
println(testerr(parseInter, "((lambda () (+ 2 2)))")) # numval 4
println(testerr(parseInter, "(collatz -1)")) # Error
println(testerr(parseInter, "(/ 1 0)")) #Error
"""
    tests_info = """1. 1 point. parse (with (x 1) x)
2. 1 point. parse (with ((x 1) (x 1)) 1)
3. 1 point. parse (lambda (x x) 1)
4. 1 point. calc (- 1 2)
5. 1 point. calc (with ((x 1)) x)
6. 1 point. calc ((lambda (+ 2 2)) 4)
