...
function type_of_expr( ae::IsZero, env::TypeEnvironment )
  argType = type_of_expr( ae.arg, env )
  if isa( argType, NumType )
    return NumType()
  end
  throw( LispError( "arg is not a NumType" ) )
end

function type_of_expr( ae::IfB, env::TypeEnvironment )
  condType = type_of_expr( ae.condition, env )
  if isa( condType,  BoolType )
    true_branchType = type_of_expr( ae.true_branch, env )
    false_branchType = type_of_expr( ae.false_branch, env )
    if true_branchType == false_branchType
      return true_branchType
    end
    throw( LispError( "true and false branch types do not match" ) )  
  end
  throw( LispError( "conition is not a BoolType" ) )
end

function type_of_expr( ae::With, env::TypeEnvironment )
  bindingType = type_of_expr( ae.binding_expr, env )
  newEnv = CTypeEnvironment( ae.name, bindingType, env )
  return type_of_expr( ae.body, newEnv )
end

function type_of_expr( ae::Id, env::TypeEnvironment )
  while isa( env, CTypeEnvironment )
    if env.name == ae.name
      return env.value
    end
    env = env.parent
  end
  throw( LispError( "This Id does not exist" ) )
end
...
function type_of_expr( ae::FunApp, env::TypeEnvironment )
  funType = type_of_expr( ae.fun_expr, env )
  argType = type_of_expr( ae.arg_expr, env )
  if funType.arg_type == argType
    return argType
  end
  throw( LispError( "fun_exp and arg_expr types do not match" ) )
end
...
function type_of_expr( ae::NIsEmpty, env::TypeEnvironment )
  listType = type_of_expr( ae.list, env )
  if isa( listType, NListType )
    return NListType()
  end
  throw( LispError( "list must be a NListType" ) )
end

function type_of_expr( ae::NCons, env::TypeEnvironment )
  fType = type_of_expr( ae.f, env )
  if isa(fType, NumType)
    rType = type_of_expr( ae.r, env )
    if isa( rType, NListType)
      return rType
    end
    throw( LispError( "r must be an NListType" ) )
  end
  throw( LispError( "f must be a Num" ) )
end
...
CITypes.typef( "test.txt" )
# CITypes.typef("4")
# CITypes.typef("(+ 1 2)")
# CITypes.typef("(iszero nempty)")
# CITypes.typef("(ifb false (ncons 1 nempty) nempty)")
# CITypes.typef("(with x 3 ( + 1 x))")
# CITypes.typef("(lambda x : number false)")
# CITypes.typef("((lambda x : number false) false)")
# CITypes.typef("nempty")
# CITypes.typef("(nisempty false)")
# CITypes.typef("(nfirst (ncons 1 nempty))")
# CITypes.typef("(nrest nempty)")